...
exports.listMembers = async (req, res) => {
  try {
    ...
    // Basic search by name or email (qualify columns with alias 'u')
    ...
    // Count total (use same alias 'u' so where references resolve)
    const countQuery = `SELECT COUNT(*) FROM users u ${where}`;
    const countResult = await db.query(countQuery, params);
    const total = parseInt(countResult.rows[0].count, 10);

    // Fetch members with active books count
    // We'll select basic info and aggregate active checkouts
    const listQuery = `
      ...
      FROM users u
      LEFT JOIN (
        SELECT user_id, COUNT(*) AS count FROM transactions WHERE status = 'ACTIVE' GROUP BY user_id
      ) active ON active.user_id = u.user_id
      LEFT JOIN (
        SELECT user_id, COALESCE(SUM(amount),0) AS total FROM fines WHERE paid = false GROUP BY user_id
...
// Get member details (including recent transactions and fines)
exports.getMemberDetails = async (req, res) => {
  try {
    ...
    const transResult = await db.query(
      `SELECT t.*, b.title, b.author, bi.barcode
       FROM transactions t
